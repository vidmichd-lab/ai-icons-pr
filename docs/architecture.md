...
1. пользователь либо загружает локальный файл, либо вводит текстовый запрос
2. для файла frontend сохраняет локальное preview как data URL для устойчивой истории
3. если есть файл, backend загружает asset в `Krea` и запускает `SeedEdit`
4. если файла нет, backend запускает text-to-image job в `Krea` по тексту и выбранному стилю
